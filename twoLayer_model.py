...
def set_subs(problem):
    ...
    problem.substitutions['F_rad_mu']    = '-(f0*tau/Pe0)*mu_z'
...
def run_cartesian_instability(args):
    ...
    data_dir += "_Pe{}_Pr{}_tau{}_invR{}_f{}_a{}".format(args['--Pe'], args['--Pr'], args['--tau'], args['--inv_R'], args['--f0'],  args['--aspect'])
    if twoD:
    ...
    f_field['g'] = 1 - (1-f0)*(zero_to_one(z_de, 0.2, width=0.05)*one_to_zero(z_de, 1.8, width=0.05))

    Q['g'] = 0#(5/Pe0)*(1 - f0)*one_to_zero(z_de, 0.2, width=0.05)
    Q.antidifferentiate('z', ('left', (1/Pe0)), out=flux_of_z)

    T_rad_z0['g'] = -((flux_of_z) / (f0/Pe0)).evaluate()['g']
    T_ad_z['g']   = T_rad_z0.interpolate(z=Lz)['g'].min() + 1 #(1/f - 1) / (1/f)
    ...
    mu0_z['g'] = -1*zero_to_one(z_de, 1, width=0.05)
