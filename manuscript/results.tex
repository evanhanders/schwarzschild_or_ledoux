...
\todo{Maybe emphasize that this scaling is a power-law with a moderate exponent, so we expect this to happen in main-sequence stars at \emph{any} $\mS$ (e.g. it's not exponential in $\mS$, so the time-scales are always short).}
