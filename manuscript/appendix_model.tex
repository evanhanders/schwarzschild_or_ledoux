...
\editone{
    The convective cores of main sequence stars with $M_* \sim [2, 10]M_\odot$ have $\rm{Pe} = \mathcal{O}(10^6)$, $\tau \approx \rm{Pr} = \mathcal{O}(10^{-6})$, and stiffnesses of $\mS = \mathcal{O}(10^{6-7})$ (see Jermyn et al.~2022, ``An Atlas of Convection in Main-Sequence Stars'', in prep).
    While our simulation is as turbulent as possible while also achieving the long-term entrainment of the Ledoux boundary, and is qualitatively in the same regime as stars ($\rm{Pe} \gg 1$, $\rm{Pr} < 1$, $\mS \gg 1$), stars are both more turbulent and have stiffer boundaries than can be simulated with current computational resources.
}
